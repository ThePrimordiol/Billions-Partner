...
export const UserAPI = {
  register: async (data: { name: string; email: string; phone: string; password: string }): Promise<{ success: boolean; message: string }> => {
    const users = getFromStorage<any>(LOCAL_KEYS.USERS);
    ...
    users.push(newUser);
    saveToStorage(LOCAL_KEYS.USERS, users);
    
    // Auto login after registration
    localStorage.setItem(LOCAL_KEYS.CURRENT_USER, JSON.stringify({
      id: newUser.id,
      name: newUser.name,
      email: newUser.email,
      phone: newUser.phone,
  ...
  login: async (email: string, password: string): Promise<{ success: boolean; message: string; user?: any }> => {
    ...
    localStorage.setItem(LOCAL_KEYS.CURRENT_USER, JSON.stringify(userWithoutPassword));
  ...
  getCurrentUser: (): any | null => {
    const user = localStorage.getItem(LOCAL_KEYS.CURRENT_USER);
    return user ? JSON.parse(user) : null;
  },

  isLoggedIn: (): boolean => {
    return !!localStorage.getItem(LOCAL_KEYS.CURRENT_USER);
  },

  updateProfile: async (userId: number, data: Partial<any>): Promise<any> => {
    const users = getFromStorage<any>(LOCAL_KEYS.USERS);
